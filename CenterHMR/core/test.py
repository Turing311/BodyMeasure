...
from CenterHMR.core.base import *
from PIL import Image
import torchvision
from CenterHMR.utils.multiprocess import * #Multiprocess
from CenterHMR.utils.util import * #Multiprocess
...
class Demo(Base):
    ...
    def run(self, image_folder, height):
        result = ""
                ...
                if self.save_mesh:
                    ...
                    for idx, vid in enumerate(vids_org):
                        ...
                        for subject_idx, batch_idx in enumerate(verts_vids):
                            save_obj1(outputs['verts'][batch_idx].detach().cpu().numpy().astype(np.float16), self.smplx.faces_tensor.detach().cpu().numpy(),obj_name.replace('.obj', '_{}.obj'.format(subject_idx)))
                            result = extract_measurements(height, outputs['verts'][batch_idx].detach().cpu().numpy().astype(np.float16))
                            break
    ...
    def process_video(self, video_file_path=None):
        ...
        import keyboard
        from CenterHMR.utils.demo_utils import OpenCVCapture, frames2video
    ...
    def webcam_run_local(self, video_file_path=None):
        ...
        print('run on local')
        import keyboard
        from CenterHMR.utils.demo_utils import OpenCVCapture, Open3d_visualizer, Image_Reader
        capture = OpenCVCapture(video_file_path)
